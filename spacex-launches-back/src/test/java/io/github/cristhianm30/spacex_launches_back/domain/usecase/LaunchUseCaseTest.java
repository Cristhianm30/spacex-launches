...
class LaunchUseCaseTest {
    ...
    @Test
    void getLaunches_ShouldReturnPaginatedLaunches() {
        // Given
        Pageable pageable = new Pageable(0, 1);
        Page<LaunchModel> page = new Page<>(Collections.singletonList(successfulLaunch), 0, 1, 1);
        when(repositoryPort.findAll(any(), any(Pageable.class))).thenReturn(page);

        // When
        Page<LaunchModel> result = launchUseCase.getLaunches(null, pageable);

        // Then
        assertEquals(1, result.getTotalElements());
        assertEquals(successfulLaunch, result.getContent().get(0));
    }
